...
def upload_video_files(file_paths: list, upload_url="http://192.168.1.129:8000/upload-movie"):
    """
...
def get_valid_video_files(wakeup_time: datetime, window: int = WINDOW, wait_seconds=20):
    """
...
def audio_listener(stop_event: threading.Event):
    ...
    trigger_phrases = ["블랙박스", "블랙박스야", "블박아"]
...
def main():
    ...
    #     response = requests.post("http://192.168.1.129:8000/upload-movie", files=files, data=data)
